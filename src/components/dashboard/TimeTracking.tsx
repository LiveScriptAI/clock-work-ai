...
const TimeTracking: React.FC<TimeTrackingProps> = ({
  ...
  return <Card className="mb-6 bg-slate-50 rounded-xl">
      ...
      <CardContent>
        {startTime && <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-md">
            ...
            {breakIntervals.length > 0 && <div className="mt-2 pt-2 border-t border-green-200">
                <div className="text-sm text-blue-600 font-medium mb-2">Break Intervals:</div>
                {breakIntervals.map((interval, index) => <div key={interval.start.toISOString()} className="text-sm text-blue-600 mb-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Button size="lg" className="bg-green-600 hover:bg-green-700 disabled:bg-gray-400 disabled:hover:bg-gray-500" onClick={handleStartShift} disabled={isShiftActive || isShiftComplete}>
            {isShiftActive ? t('Shift Started') : t('Start Shift')}
          </Button>
          
          <Button variant="default" size="lg" onClick={handleBreakToggle} disabled={!isShiftActive || isShiftComplete} className="px-8 bg-blue-600 hover:bg-blue-700">
            {isBreakActive ? t('End Break') : t('Start Break')}
          </Button>
          
          <Button size="lg" className="bg-red-600 hover:bg-red-700" onClick={handleEndShift} disabled={!isShiftActive || isShiftComplete}>
            {t('End Shift')}
          </Button>
        </div>
      </CardContent>
